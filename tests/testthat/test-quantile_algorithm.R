...
cdf <- get_cdf(d)
tau <- c(0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5, NA, NaN, 0.55, 0.6, 0.9, 1.0)
...
test_that("quantiles by inversion of cdf works", {
	expect_equal(
		sapply(tau, function(x) {
			left_inverse(cdf, at = x, low = -1, high = 7, tol = 1e-6, maxiter = 1000)
		}),
		ans
	)
	...
	expect_identical(eval_quantile(d, at = c(NA_real_, NA_real_)), c(NA_real_, NA_real_))
})
