...
test_that("range for gpd dst works", {
	...
	expect_equal(range(dst_gpd(-4.56, 5.48, -654)), c(-4.56, (-4.56 - (5.48 / -654))))
})
...
test_that("range for finite dst works", {
	...
	expect_equal(range(dst_finite(c(-1,2,654,66,5), probs = c(0.1,0.3,0.5,0.05,0.05))), c(-1, 654))
	expect_equal(range(dst_finite(c(-1,-2,-5.565,-1.1,0), probs = rep(0.2,5))), c(-5.565,0))
})
