...
plot.dst <- function(object,
	...
	if (identical(fname, "quantile")) {
		...
		f <- get_quantile(object)
		ellipses[["expr"]] <- as.name("f")
		do.call(graphics::curve, args = ellipses)
	}
	if (is.null(ellipses[["from"]])) {
		q0 <- eval_quantile(object, at = 0)
		if (identical(q0, -Inf)) {
			ellipses[["from"]] <- eval_quantile(object, at = 0.001)
		} else {
			ellipses[["from"]] <- q0
		}
	}
	if (is.null(ellipses[["to"]])) {
		q1 <- eval_quantile(object, at = 1)
		if (identical(q1, Inf)) {
			ellipses[["to"]] <- eval_quantile(object, at = 0.999)
		} else {
			ellipses[["to"]] <- q1
		}
	}
