...
You fit a supervised learning model for the mean of Y|X -- that's your priority. A secondary priority is to get a sense of the uncertainty in Y|X. You'd like to make a log-normal approximation to the distribution of Y|X.
...
m <- predict(model)[1]
dst_lnorm() %>% 
  set_mean(m)   # Then, resolve via MLE
```
...
This might not be so useful in a univariate distribution setting, but would be if we're specifying a multivariate distribution, and want to keep a measure of dependence constant between two pairs (X1, X2) and (X2, X3).

```
dst_norm(mean = a, variance = a)   # Then, resolve via MLE.
```


**3**

Restrict a parameter: you'd like to fit a GPD, and are confident that the distribution is heavy tailed, so would like the shape parameter to be >0.
